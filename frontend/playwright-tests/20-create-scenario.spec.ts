...
import { loginToSite, readTestUserFromFile, TestUser, waitForTextInputToSettle } from './testutils';
dotenv.config();
...
test.describe('Generate and edit scenario workflows', () => {
  ...
  test.describe('Scenario creation flow', () => {
    ...
    //   await page.getByRole('button', { name: 'Save', exact: true }).click();
      ...
      await page.locator('div').filter({ hasText: /^Scenario Title$/ }).getByRole('button').nth(1).click( { delay :1500 });
      await page.locator('.icon-button.icon-button--m.icon-button--primary.ai-textarea__ai-button').first().click( { delay :1500 });
