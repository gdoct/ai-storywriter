...
export function createSimilarScenarioPrompt(
  ...
  prompt += "====== EXISTING SCENARIO (JSON) ======\n```json";
  prompt += JSON.stringify(existingScenario, null, 2) + "\n\n";
  ...
  if (selections.retainCharacters) {
    prompt += "* IMPORTANT: KEEP existing characters. \n";
    prompt += "* ADD at least 1 new character\n";
  } else {
    prompt += "* CREATE NEW characters for the scenario (don't use any existing characters)\n";
  }

  prompt += "\n";
  prompt += "✗ NEVER RETAIN any of the other fields in the scenario. instead look at the schema below and generate new content based on it.\n\n";
  ...
  prompt += "IMPORTANT: Provide ONLY the JSON object - no explanations, commentary, or additional text. Ensure the JSON is valid and properly formatted.";
